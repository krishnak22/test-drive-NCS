...
aws iam put-role-policy \
  --role-name "$ROLE_NAME" \
  --policy-name "test-drive-ncs-bf-policy-1" \
  --policy-document file://"$POLICY_FILE_1"

# Attach the second inline policy to the IAM role using the second policy JSON file
aws iam put-role-policy \
  --role-name "$ROLE_NAME" \
  --policy-name "test-drive-ncs-bf-policy-2" \
  --policy-document file://"$POLICY_FILE_2"


echo "Both inline policies have been successfully attached to the service account's IAM role."
